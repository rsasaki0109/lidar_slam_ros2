...
non-GPL default workflow from `v0.2.x` and adds: a Livox MID-360 operator
toolkit (recording → SLAM → Autoware
...
- lanelet generation is out of scope as a public default; the
  `simple_lanelet2_generator.py` is a tutorial-grade tool,
...
loop closure. The triangle descriptor is
additive — it does not change the default public path.
