...
- `output/classic_path_report_20260326.md`
...
So the classic path still needs work, but the report now makes the direction
clear: backend GNSS helps substantially, while the current packet IMU path on
`driving_30_kmh` is not yet a default recommendation.
