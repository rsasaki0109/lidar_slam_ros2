...
| exact displayed S6 product-shell command | command-contract implementation `0633c2a` plus README chooser `8a8876a`; an ordinary unsourced shell restores `${ROS_DISTRO:-jazzy}` before importing `rosbag2_py` and passes all 36 graph docs/product-CLI tests; deleting the prelude now fails plan validation before a reviewer sees the card |
| focused graph product/docs regressions | 36 passed in a Jazzy-sourced isolated package process, including the three goal/first-step/boundary routes; an additional 15 diagnosis regressions cover all five user-reported map symptoms and missing-bag/root-cause boundaries |
