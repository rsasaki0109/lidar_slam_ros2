...
| exact displayed S6 product-shell command | command-contract implementation `0633c2a` plus three-goal entrypoint tips `8a8876a` and `1e56431`; an ordinary unsourced shell restores `${ROS_DISTRO:-jazzy}` before importing `rosbag2_py` and passes all 37 graph docs/product-CLI tests; deleting the prelude now fails plan validation before a reviewer sees the card |
| focused graph product/docs regressions | 37 passed in a Jazzy-sourced isolated package process, including matching README/Getting Started first-goal, safe-action, version, write, storage, and time boundaries plus retained advanced actions; an additional 15 diagnosis regressions cover all five user-reported map symptoms and missing-bag/root-cause boundaries |
