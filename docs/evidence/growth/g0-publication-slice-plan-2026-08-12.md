...
| safe operator interruption | original implementation `6d1249e`, real-ROS correction `0301a0d`, concise-stop follow-up `181b251`; the stable CLI waits and signals the complete isolated runner group, while the dogfood shell exits SIGINT/SIGTERM as 130/143 without falling through to a false timeout; the second exact-tip Jazzy trial entered the real offline wait, sent one Ctrl-C, returned 130 in 1.5 seconds, reaped every descendant, preserved the schema-valid interrupted evidence and one-action card, reduced stderr 9,223 → 1,845 bytes (80.00%), and emitted zero timeout labels or launch-log dumps; dogfood 14, lower runner 48, docs/product 42, support/installed 25, option 21, broad S6 332, four schemas, and 17 checksums pass | genuine timeout diagnostics remain; the public-fixture trial used an explicit 0.5 GiB floor on a controlled overlay, so the unchanged default storage requirement remains unsatisfied and this is not a complete map, clean-host result, paired GLIM observation, parity/superiority claim, upload, network access, issue, or GitHub write |
