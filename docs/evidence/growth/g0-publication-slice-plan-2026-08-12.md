...
| safe operator interruption | original implementation `6d1249e`, real-ROS correction `0301a0d`, concise-stop follow-up `181b251`; the stable CLI waits and signals the complete isolated runner group, while the dogfood shell exits SIGINT/SIGTERM as 130/143 without falling through to a false timeout; the concise trial returned 130 in 1.5 seconds, reaped every descendant, reduced stderr 9,223 → 1,845 bytes (80.00%), and emitted zero timeout labels or launch-log dumps; the later exact-installed `edff76d` run omitted the storage override, passed 7.01 GiB free / 5.00 GiB required, observed live RKO-LIO and graph nodes, and returned 130 in 1.43 seconds with every descendant absent, four schemas and eight early-interruption checksums valid, one ACTION REQUIRED / Next / Details, and no timeout, traceback, success card, or `map.pcd` | the unchanged default-storage interruption gate now passes; genuine timeout diagnostics remain, while controlled-overlay, clean-host/package-manager, complete-map quality, paired GLIM, parity/superiority, upload, network, issue, and GitHub-write claims remain unavailable |
