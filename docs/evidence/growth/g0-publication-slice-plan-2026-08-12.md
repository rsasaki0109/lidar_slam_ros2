...
| exact displayed S6 product-shell command | command-contract implementation `0633c2a`, three-goal entrypoint tips `8a8876a`/`1e56431`, and honest FAIL intake `da99c7e`; an ordinary unsourced shell restores `${ROS_DISTRO:-jazzy}` before importing `rosbag2_py` and passes all 38 graph docs/product-CLI tests; deleting the prelude now fails plan validation before a reviewer sees the card |
| focused graph product/docs regressions | 38 passed in a Jazzy-sourced isolated package process, including matching README/Getting Started first-goal boundaries, retained advanced actions, and parsed issue-form PASS/FAIL/receipt/privacy semantics; an additional 15 diagnosis regressions cover all five user-reported map symptoms and missing-bag/root-cause boundaries |
