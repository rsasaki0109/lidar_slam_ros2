...
| safe operator interruption | exact implementation `6d1249e`; `start` supervises the delegated runner in an isolated session, forwards one Ctrl-C as SIGINT, waits up to 20 seconds for process-group cleanup and terminal evidence, then requests termination for 10 seconds before a forced reap; a real synthetic process probe returns 130 with its child absent, and the retained interrupted manifest becomes one `workflow-interrupted` ACTION REQUIRED card with one Next and no traceback; 42 sensor-setup regressions, exact S3 lifecycle 77 and edit/merge 15, all 48 lower map-runner regressions, plus 42 docs/product, 25 support/installed, 21 option-contract, and 331 broad S6 regressions pass with changed-code `ament_flake8`, bytecode/JSON/patch hygiene, plan validation, and strict MkDocs | one stop request becomes a bounded evidence-preserving handoff instead of an ambiguous Python interruption; the real subprocess probe is not a ROS mapping run, clean-host timing result, paired GLIM observation, or parity/superiority claim; no map, upload, network access, issue, or GitHub write is performed |
