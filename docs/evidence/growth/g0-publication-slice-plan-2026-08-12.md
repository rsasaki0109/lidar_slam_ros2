...
| safe operator interruption | original implementation `6d1249e`, process correction `0301a0d`, timeout-label correction `181b251`, evidence-bound summary `8370ac5`; only a sealed manifest whose status, stage, two exit codes, and signal error agree receives the concise direct-run stop/evidence line, while plain 130 and inconsistent evidence retain full failure diagnostics; `start` suppresses only its duplicate workflow-interrupted generic error; the exact-installed default-storage trial passed 7.01 GiB free / 5.00 GiB required, observed live RKO-LIO and graph nodes, and one Ctrl-C returned 130 in 1.38 seconds with every descendant absent; post-stop generic map failure, failed-command repetition, generic session error, false timeout, log dump, traceback, and success card are all zero; stderr fell 1,971 → 1,200 bytes (39.12%), four schemas and eight checksums pass, and one ACTION REQUIRED / Next / Details remains | expected stops are concise without weakening ordinary failure or genuine timeout diagnostics; controlled-overlay, clean-host/package-manager, complete-map quality, paired GLIM, parity/superiority, upload, network, issue, and GitHub-write claims remain unavailable |
