...
| published starter dependency gate | 63 queue regressions pass; the schema-valid live card keeps #422 visible but ineligible under `WAITING_FOR_PUBLIC_GATES`, preserves unrelated starter eligibility, prioritizes duplicate review then one independent local maintainer preview, binds its exact title/labels/heading-free body and three digests, follows the stable issue number across a title edit, rejects body/task tampering, arbitrary gate commands, or write authority, and performs only GET-only GitHub reads |
