...
| S2 first-map foundation | 34 | one bounded demo/own-bag route reaches a verified local 3D result, retains recovery state, diagnoses recorded Odometry-to-TF connectivity and replay-order future gaps, returns the exact shortage plus preserved retry command on low storage, and keeps direct expert execution detail available while guided starts show only the profile, storage decision, honest progress, and terminal handoff | public Humble/Jazzy CI |
| S3 map lifecycle | 25 | setup, history, compare, edit, merge, and support preserve provenance and receipts; confirmed starts move from one calibration decision through one concise product-owned progress stream to one terminal card, with exactly one safe `Next` on success or failure and complete structured evidence retained | focused local review |
...
| single-card map completion | exact implementation `8a620e5`; successful `start` now emits one `Map session: VERIFIED` or `UNVERIFIED` card containing map output, verification, viewer, session index/page, evidence paths, and exactly one recommended `Next`; the former completion/Reopen/summary duplication is removed, viewer failure makes that one action the view retry without changing map status, and session-index failure retains one completed fallback card; 37 sensor-setup regressions, exact S3 lifecycle 72 and edit/merge 15, plus 42 docs/product, 25 support/installed, 21 option-contract, and 331 broad S6 regressions pass with changed-code `ament_flake8` and strict MkDocs | a finished map ends in one status and one next action while JSON, progress, recovery, and evidence authority remain unchanged; no real mapping, upload, network access, issue, or GitHub write is performed |
...
| safe operator interruption | original implementation `6d1249e`, process correction `0301a0d`, timeout-label correction `181b251`, evidence-bound summary `8370ac5`, quiet native-reader follow-up `d0e3361`, concise guided-start follow-up `3dcca0c`; product-dispatched native bag opens suppress only routine INFO and a guided `start` gives lower execution layers one concise-output boundary while direct expert `run` and dogfood output remain complete; the exact-installed default-storage trial passed 6.99 GiB free / 5.00 GiB required, observed live RKO-LIO and graph nodes, and one Ctrl-C returned 130 in 1.543 seconds with every descendant absent; guided stdout fell 3,679 → 1,448 bytes (60.64%) to 23 lines, keeps profile/storage before mapping readiness and one progress/terminal stream, and omits internal commands, paths, YAML, topics, frames, staging promises, lifecycle duplication, and launch-log location; stderr contains only the stop/evidence pair; generic failure, failed command, generic session error, false timeout, log dump, traceback, and success cards stay at zero; four schemas and 18 checksums pass | guided starts are concise without discarding stderr or weakening ordinary failure, warnings, direct expert logging, or genuine timeout diagnostics; controlled-overlay, clean-host/package-manager, complete-map quality, paired GLIM, parity/superiority, upload, network, issue, and GitHub-write claims remain unavailable |
