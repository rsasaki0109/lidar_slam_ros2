...
| S3 map lifecycle | 25 | setup, history, compare, edit, merge, and support preserve provenance and receipts; confirmed starts move from one calibration decision through durable progress to one terminal card, with exactly one safe `Next` on success or failure and complete structured evidence retained | focused local review |
...
| safe operator interruption | original implementation `6d1249e`, process correction `0301a0d`, timeout-label correction `181b251`, evidence-bound summary `8370ac5`, quiet native-reader follow-up `d0e3361`; product-dispatched native bag opens temporarily set only the previously-unset `rosbag2_storage` logger to WARN, restore it after open, preserve direct and explicitly configured logging plus WARN/ERROR, and replace four duplicate reader-open sites with one boundary; the exact-installed default-storage trial passed 7.02 GiB free / 5.00 GiB required, observed live RKO-LIO and graph nodes, and one Ctrl-C returned 130 in 1.545 seconds with every descendant absent; six routine storage INFO lines fell to zero and stderr fell 1,200 → 180 bytes (85.00%), while the stop/evidence pair and one ACTION REQUIRED / Next / Details remain; generic failure, failed command, generic session error, false timeout, log dump, traceback, and success cards stay at zero; four schemas and 18 checksums pass | expected stops are quiet without discarding stderr or weakening ordinary failure, warnings, explicit expert logging, or genuine timeout diagnostics; controlled-overlay, clean-host/package-manager, complete-map quality, paired GLIM, parity/superiority, upload, network, issue, and GitHub-write claims remain unavailable |
