...
>
> Planned follow-up inventory: 319 paths; SHA-256
...
| --- | ---: | --- | --- |
| S1 runtime safety | 15 | unsafe point-cloud and VoxelGrid layouts, plus readable samples with empty frame IDs, fail closed without losing valid fields; a rejected map update preserves the movement threshold for an immediate safe retry, and asynchronous exceptions stay inside the component boundary | public Humble/Jazzy CI |
| S2 first-map foundation | 33 | one bounded demo/own-bag route reaches a verified local 3D result, retains recovery state, diagnoses recorded Odometry-to-TF connectivity, and returns the exact shortage plus preserved retry command on low storage | public Humble/Jazzy CI |
...
4. exact 319-path coverage with the fixed inventory digest;
...
| Check | Result |
| --- | --- |
| exact Git-derived plan check | `PLAN_VALID_LOCAL_ONLY`; 319 paths, 7 slices, no remote mutation |
...
| clean candidate release bundle | exact-head reproducibility rehearsal passes at NTU attached-storage validation carrier `b01b251`; two byte-identical and reverified bundles each contain 267 manifest files, total 11,959,011 archive bytes, and have SHA-256 `f963391bf76f67e27828bf0c8eadada484ac8b8b9481e5f904fcec72e1c64bad`; archived NTU/storage help and all four RTK identities execute; this supersedes the 264-file result as local evidence only |
| complete maintained Python gate | NTU attached-storage synchronization: source-explicit graph 1,477 passed / 13 skipped / 11 existing ImageIO warnings; source-explicit lidarslam 1,040 passed; 2,517 total; strict MkDocs and changed-file Jazzy `ament_flake8` pass; latest public `39b5e38` Actions pass 10 successful / 4 intentional skips / 0 failures / 0 pending; the exact-carrier bundle rehearsal passes without granting mount, tag, or publication authority |
