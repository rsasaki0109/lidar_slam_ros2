...
| safe operator interruption | original implementation `6d1249e`, real-ROS correction `0301a0d`; a controlled red trial exposed the outer dispatcher killing `start` and leaving a surviving successful workflow with stale `running` session, so the stable CLI now waits and the helper signals the complete isolated runner group; the exact corrected-tip Jazzy trial observed `workflow_running`, sent one Ctrl-C, returned 130 in 1.5 seconds, reaped every descendant, sealed `interrupted / complete`, and emitted one ACTION REQUIRED / Next / Details with no traceback or success card; four schemas and 17 artifact checksums pass, plus 51 CLI/sensor, exact S3 77, lower runner 48, docs/product 42, support/installed 25, option 21, and broad S6 332 regressions | the public-fixture trial used an explicit 0.5 GiB floor on a 1.58 GiB-free controlled overlay; the unchanged default storage requirement remains unsatisfied, and this is not a complete map, clean-host result, paired GLIM observation, parity/superiority claim, upload, network access, issue, or GitHub write |
