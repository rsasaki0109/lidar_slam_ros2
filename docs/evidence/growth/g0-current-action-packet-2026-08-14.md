...
> `95aaa419bd860ee37c3f711cdfd4bf2d9c54c700`
>
...
| --- | --- | --- |
| Draft PR #427 | open, draft, and mergeable; capture-time public baseline `3ed632e…` remains the frozen review anchor; exact public head `95aaa41…` includes receipt-bound onboarding evidence and synchronized inventory; the one-line Jazzy CI repair is local at `b87c193…` | no merge, Pages deployment, cohort launch, or E2 authority is implied |
| Exact public PR-head CI | exact public head `95aaa41…` completed with 7 successful checks, 6 dependency/publication skips, and 1 failed Jazzy default workflow; the failure is isolated to I100 standard-library import order in `test_run_source_onboarding_probe.py`, while Humble and both Docker builds pass | exact Jazzy `ament_flake8` now checks all 90 Python files with no problem at local `b87c193…`; the local repair still needs its own exact-head public CI and green CI would not grant release/E2 authority |
...
| Complete product gate through public head | graph 1,442 passed / 13 skipped / 11 existing ImageIO warnings; lidarslam 998 passed; registered CTest passes 93 / 93 for `lidarslam` and 232 / 232 for `graph_based_slam`; strict MkDocs passes locally; exact public `95aaa41…` has 7 successful / 6 skipped / 1 failed check because Jazzy enforces one import-order detail | local `3c19824…` adds 250 focused G0/candidate/onboarding/deployment regressions, strict docs, and a reproducible 253-file bundle (`e3fbe1cf…`, 11,611,987 bytes); `b87c193…` then passes the related 24 tests and exact Jazzy `ament_flake8` across all 90 files, but still needs replacement exact-head public CI |
