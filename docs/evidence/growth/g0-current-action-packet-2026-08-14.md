...
The status-specific operator handoff now prints the trusted settings URL and
...
| --- | --- | --- |
| Draft PR #427 | open and draft at capture-time public baseline `2a92cc5…`; guided readiness is public, while protected-environment preflight tip `adecca6…` and its 263-path inventory are the reviewed local follow-up | the new source tip still requires its own public CI; the preflight changes no E2 authority |
