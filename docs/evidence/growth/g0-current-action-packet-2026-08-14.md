...
| G0 product-Draft dependency audit | exact implementation `e08ec9c…` plus the current local branch-update UX follow-up; 10 focused regressions cover local-head/public-head identity, bounded latest-check selection, fail-closed drift/CI/authority cases, divergent history, and a verified fast-forward handoff; an authenticated GET rehearsal observed public head `4b2ab514…`, pre-increment local tip `77c36ac…`, and `non_force_update_possible: true` | head drift no longer loops back to the same audit: the next action binds both exact tips, emits no push command, keeps push/force/write authority false, requires a separate exact-tip non-force decision, and reserves the GET-only audit for post-update verification; only observed `MERGED` permits the environment gate to become next |
