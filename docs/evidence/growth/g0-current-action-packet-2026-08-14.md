...
| Safe operator interruption | original implementation `6d1249e…`, real-ROS correction `0301a0d…`; a red trial proved the former outer dispatcher could kill the `start` helper and leave a surviving workflow plus stale `running` session, while the correction makes the stable CLI wait and signals the complete isolated runner group; the exact corrected-tip Jazzy trial observed `workflow_running`, sent one Ctrl-C, returned 130 in 1.5 seconds, reaped every descendant, sealed `interrupted / complete`, and emitted one ACTION REQUIRED / Next / Details with no traceback or success card; four schemas and 17 artifact checksums pass, plus 51 CLI/sensor, S3 77, lower runner 48, docs/product 42, support/installed 25, option 21, and broad S6 332 regressions | the local public-fixture trial used an explicit 0.5 GiB floor on a 1.58 GiB-free controlled overlay; the unchanged default storage requirement remains unsatisfied, and this is not a complete map, clean-host result, paired GLIM observation, parity/superiority claim, upload, network access, issue, or GitHub write |
