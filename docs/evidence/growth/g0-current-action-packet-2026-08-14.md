...
| S1 rejected-map-update recovery | exact implementation `99cce93…`; one pure commit-state regression and the real asynchronous component prove that an unsafe update crossing a positive 0.02 m threshold leaves the threshold available to the same-geometry safe retry; the component case passes 10 / 10 independent Jazzy processes, scanmatcher CTest passes 10 / 10, and the exact displayed S1 command reports 4,253 cases / 0 errors / 0 failures / 127 skips | map position and cumulative submap distance advance only after success; worker setup, future, and map-update exceptions remain inside the component boundary; current Humble proof still requires exact-head public CI, so issue #69 and release authority remain open |
...
| Complete maintained product gate | exact implementation `99cce93…`: graph 1,452 passed / 13 skipped / 11 existing ImageIO warnings; lidarslam 1,034 passed; 2,486 total; strict MkDocs and focused C++ formatting/lint pass; byte-identical rehearsals produce a 261-file, 11,931,414-byte bundle with SHA-256 `5f8429e4038ca6567b2bbdb0bb00e36e5c08160631ad30ff77c7422f5080f345`; latest public `7b3cb99…` Actions pass 10 successful / 4 intentional skips / 0 failures / 0 pending | the S1 threshold-retry implementation is public and the S6 ordinary-shell review repair is the next local increment; registered CTest passes 94 / 94 for `lidarslam` plus 232 / 232 for `graph_based_slam`, and the exact displayed S1 command passes 4,253 test cases with 127 skips |
...
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 304 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `c8cef196cc20780c0551e1797490442d6ebe55aea7f8fc6a928920ace73482b7` | the bounded visual-symptom and privacy-safe support handoff files belong to S6; every displayed verification group is now self-contained for an ordinary terminal; exact coverage and the digest add no GitHub, release, posting, or community authority |
...
E4 v0.9.1 release/images: DEFER — G0 matrix and distribution gates remain open; five exact-head blocking benchmark profiles currently have NO_DATA
```
