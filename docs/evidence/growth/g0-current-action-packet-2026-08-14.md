...
| G0 product-Draft dependency audit | exact implementation `e08ec9c…` plus the current local branch-and-description UX follow-up; 16 focused regressions cover local-head/public-head identity, bounded latest-check selection, fail-closed drift/CI/authority cases, divergent history, unavailable-history recovery, a verified fast-forward handoff, canonical body hashing, stale-body review blocking, exact compare lineage, and safe review-map labels; an authenticated GET rehearsal observed public head `4b2ab514…`, 300 public commits, 374 public changed files, and a PR body still describing the older 321-path / 77-follow-up-commit packet | head drift no longer loops back to the same audit: the next action binds both exact tips and the canonical repository URL, emits no push or edit command, and pairs the separate exact-tip non-force decision with a separately authorized exact-body refresh; the body carries current whole-PR/P2 budgets, evidence counts, three exact phase links, and seven bounded slice rows, requires branch verification first, keeps Draft/review/mark-ready/merge/write authority false, and its digest must match before exact review; only observed `MERGED` permits the environment gate to become next |
