...
| Complete maintained product gate | exact S6 evidence carrier `72a8c9e…`: source-explicit graph 1,452 passed / 13 skipped / 11 existing ImageIO warnings; source-explicit lidarslam 1,036 passed; 2,488 total; strict MkDocs and changed-file Jazzy `ament_flake8` pass; byte-identical rehearsals produce a 261-file, 11,932,456-byte bundle with SHA-256 `436586241b9475653100caab5dd7dd98bcbbad186287847f9b0a7176a93aa1fb`; latest public `7b3cb99…` Actions pass 10 successful / 4 intentional skips / 0 failures / 0 pending | the S1 threshold-retry implementation is public and the S6 ordinary-shell review repair is complete locally; the exact displayed S1 command passes 4,253 test cases with 0 errors / 0 failures / 127 skips, and the repaired S6 command passes 35 / 35 from an unsourced shell |
...
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 311 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `df173015a6fc399e15df05794ea126196437f333521ef4db546804ca3930def0` | the bounded support handoff and compressed-bag playback evidence paths belong to S6; every displayed verification group is self-contained for an ordinary terminal; exact coverage and the digest add no GitHub, release, posting, or community authority |
