...
| Actionable stable-release profile gate | historical Leo implementation `92bb524…` remains carrier-bound at 0.139152 m and cannot be copied to the current candidate; exact timing carrier `4bdd7ec…` correctly reports five blocking `NO_DATA` profiles—Newer College Maths, NTU VIRAL, both RTK-SLAM Construction sequences, and Leo Drive—and exits 2 with one remediation per row | current exact-head evidence remains incomplete; no historical metric is relabeled, and no tag, Release, image, or E4 authority is granted |
...
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 321 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `f378fd654e54595dce5da1e5763374f3f56db1dcd87513ab3c5e4de6afcbdbd5` | the Odometry/TF connectivity and timing preflight belongs to S2/S6, while NTU and RTK acquisition remain in S5; every displayed verification group is self-contained for an ordinary terminal; exact coverage and the digest add no mount, GitHub, release, posting, or community authority |
