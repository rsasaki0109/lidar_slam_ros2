...
| --- | --- | --- |
| Draft PR #427 | open, draft, and mergeable; capture-time public baseline `3ed632e…` remains the frozen review anchor; latest observed exact public head before this local increment is `7b3cb99…` | no merge, Pages deployment, cohort launch, or E2 authority is implied; the GET-only audit grants no mark-ready or merge authority |
| Exact public PR-head CI | capture-time public baseline remains **PASS** for `3ed632e…`: 10 successful checks plus 4 intentionally skipped non-publication jobs, 0 failures; exact public `7b3cb99…` is also **PASS** with 10 successful / 4 intentional skips / 0 failures / 0 pending | Humble/Jazzy default workflows, Docker verification builds, upgrade checks, docs/metadata, candidate contract, and release-readiness guards all passed; publication jobs stayed skipped by design |
| G0 product-Draft dependency audit | exact implementation `e08ec9c…` plus the current local branch-update UX follow-up; 11 focused regressions cover local-head/public-head identity, bounded latest-check selection, fail-closed drift/CI/authority cases, divergent history, unavailable-history recovery, and a verified fast-forward handoff; an authenticated GET rehearsal observed public head `4b2ab514…`, pre-increment local tip `77c36ac…`, and `non_force_update_possible: true` | head drift no longer loops back to the same audit: the next action binds both exact tips and the canonical repository URL, never trusts a checkout-specific `origin`, emits no push command, keeps push/force/write authority false, requires a separate exact-tip non-force decision, and reserves the GET-only audit for post-update verification; only observed `MERGED` permits the environment gate to become next |
| G0 publication-slice verification | all seven review cards plus one compact PR overview pass exact three-phase lineage, commit composition, 380-path union, schema, and authority checks; 30 checker regressions cover bounded human/JSON overview output, mutually exclusive modes, missing/extra phase paths, ordinary-shell ROS recovery, separate package pytest processes, cache suppression, and recognized direct remote-write CLI refusal; the exact S6 product-shell command passes 35 / 35 from an unsourced shell and the copy-ready S1 command reports 4,253 cases / 0 errors / 0 failures / 127 skips | the overview makes the large Draft scannable without dumping 331 follow-up paths into its summary; reviewers drill into each exact slice, while the local review still grants no push, review submission, mark-ready, merge, environment, release, posting, or community authority |
...
| Canonical NDT upstream Draft preflight | refreshed 2026-08-16 as `READY_FOR_DRAFT_PR`, 30 / 30 PASS; exact upstream `5495fd9…`, exact candidate `618f02f6…`, expected fork verified, proposed branch absent, 4 open PRs inspected, 0 duplicates, 0 API errors, and write authority false; the `3e11f30…` response packet is `BLOCKED` solely because that Draft URL is absent and emits no reply body | the local candidate and replies are technically bound but unpublished; this neither creates nor authorizes an upstream branch, PR, or rosdistro comment |
...
| E1 source review | product Draft #427 is public and exact-head CI PASS; canonical NDT Draft remains local `READY_FOR_DRAFT_PR` | publishing the third-party upstream branch/PR and later replies requires a separate exact-revision decision; no merge is implied |
...
1. Run the GET-only product-Draft audit, verify the composed initial / CI-bridge
   / follow-up review coverage, then review the exact matching head by all seven
   publication slices. Stop on branch, commit, mergeability, CI, phase lineage,
