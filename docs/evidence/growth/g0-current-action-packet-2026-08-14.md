...
| G0 publication-slice verification | exact follow-up `0633c2a…`; all seven review cards pass schema and authority checks; 20 checker regressions now cover ordinary-shell ROS recovery for both the lifecycle and product-shell commands, separate package pytest processes, cache suppression, and recognized direct remote-write CLI refusal; the exact S6 product-shell command passes 35 / 35 from an unsourced shell and the copy-ready S1 command reports 4,253 cases / 0 errors / 0 failures / 127 skips | the S6 command previously failed 2 / 35 because `rosbag2_py` was unavailable before the reviewer sourced ROS; that hidden shell dependency is now regression-bound, and the local review still grants no push, merge, environment, release, posting, or community authority |
...
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 330 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `807c2bec5fce302c92f500e80041eff6b47d921e090ade8e91930568472f6877` | the Odometry/TF connectivity and timing preflight belongs to S2/S6, while NTU and RTK acquisition remain in S5; the weekly growth snapshot, proposal content, source-bound no-write issue-triage packet, and paired public-identity preparation schema belong to S6; every displayed verification group is self-contained for an ordinary terminal; exact coverage and the digest add no mount, GitHub, release, posting, or community authority |
