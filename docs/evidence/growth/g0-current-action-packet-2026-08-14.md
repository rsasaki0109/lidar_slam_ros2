...
>
> Latest exact public Draft head before this local increment:
...
>
> Latest copy-ready G0 slice verification tip:
> `297115d14ea0a979ee0043e24d55a2a80746e382`
>
> Latest S1 rejected-map-update threshold recovery tip:
> `99cce93a07a7cc136eb925c446dd705bdcd7b37c`
>
> Latest S6 ordinary-shell review recovery tip:
> `0633c2a604489538e0f087c02385e7c6467540c3`
>
> Latest S6 review evidence carrier:
...
>
> Latest exact-head CI import-order repair tip:
> `b87c1936c1799834288d6d2c39aa616ab15c348d`
>
...
>
> Latest paired usability recorder tip:
...
> Latest dependency-gated contributor next-action tip:
> `76be89576ae544d77e661bc5d098b5087b497c5c`
>
> Latest independent starter-review tip:
> `e4ce0aa6eb7d53423423a02af65f923472708f44`
>
> Latest exact starter-publication-handoff tip:
...
> `4163b8c6f0ebb63504a0de398d9654333bf3919e`
>
> Latest Odometry-to-TF bag-preflight tip:
> `402c23765fe125a2f42d7fd245d2a1c972a1ab34`
>
...
authority remain explicit. The latest
...
393-path / three-phase / seven-slice coverage. A dirty checkout instead selects
...
| G0 product-Draft dependency audit | exact implementation `e08ec9c…` plus the current local branch, description, role-routing UX, and anonymous-ledger follow-up; 20 focused regressions cover local-head/public-head identity, bounded latest-check selection, fail-closed drift/CI/authority cases, divergent history, unavailable-history recovery, a verified fast-forward handoff, canonical body hashing, stale-body review blocking, exact compare lineage, safe review-map labels, tamper-resistant capability lanes, optional anonymous exact-head ledger summaries, retained-path exclusion, and refusal to inherit review authority; an authenticated GET rehearsal observed public head `4b2ab514…`, 300 public commits, 374 public changed files, and a PR body still describing the older 321-path / 77-follow-up-commit packet | head drift no longer loops back to the same audit: the next action binds both exact tips and the canonical repository URL, emits no push or edit command, and pairs the separate exact-tip non-force decision with a separately authorized exact-body refresh; the body carries current whole-PR/P2 budgets, evidence counts, three exact phase links, seven bounded slice rows, and four anonymous role lanes, requires branch verification first, keeps Draft/reviewer-request/review/mark-ready/merge/write authority false, and its digest must match before exact review; only observed `MERGED` permits the environment gate to become next |
| Role-based Draft review routing | four capability lanes cover all seven slices, 346 paths, and 33 verification groups exactly once; six focused routing regressions and the G0 schema reject lane drift, duplicate scope, unsafe labels, stale counts, personal-identity fields, authority claims, and bundle omission | two reviewers is an advisory capacity target, not a merge gate; the packet stores no username/email/organization and cannot request a reviewer, submit a review, mark ready, merge, or mutate GitHub |
...
| G0 publication-slice verification | all seven review cards plus one compact PR overview pass exact three-phase lineage, commit composition, 393-path union, schema, authority, and Git-numstat inventory checks; 34 checker regressions cover exact slice-budget composition, top-three textual hotspots, named binary review paths, malformed/stale numstat rejection, bounded human/JSON output, mutually exclusive modes, missing/extra phase paths, ordinary-shell ROS recovery, clean-checkout build-before-test enforcement, separate package pytest processes, cache suppression, and recognized direct remote-write CLI refusal; the exact S6 product-shell command passes 35 / 35 from an unsourced shell and the copy-ready S1 command now builds from a clean checkout and reports 3,076 cases / 0 errors / 0 failures / 126 skips | the overview makes the large Draft scannable without dumping 346 follow-up paths into its summary and tells a reviewer where the largest textual and binary deltas are; line volume is an effort hint, not proof of correctness or review completion, and the local review still grants no push, review submission, mark-ready, merge, environment, release, posting, or community authority |
...
| Contributor starter queue | exact independent-review `e4ce0aa…` and publication-handoff `0a34e72…`; C5–C9 remain `READY_LOCAL_ONLY`; 63 queue regressions, focused C5/C6 strict-MkDocs profiles, the exact 323-test S6 integration command, and 61 plan/routing/G0 regressions pass; current authenticated GET-only output finds 1 published `good first issue` (#422), 0 eligible starters, 1 blocked starter, 1 open PR, 0 potential task matches, and one digest-bound C5 handoff | the contributor still waits instead of entering the closed cohort; title, sorted labels, heading-free body, and task/queue/body digests are locally reviewable, but issue creation remains a separately confirmed external write; no issue, label, assignment, comment, PR, Pages deployment, or community post was changed |
...
1. Run the GET-only product-Draft audit, verify the composed initial / CI-bridge
   / follow-up review coverage, then choose one of the four capability lanes
   and review the exact matching head by its assigned publication slices. Stop
   on branch, commit, mergeability, CI, phase lineage,
   or inventory drift. Mark-ready and merge remain separate maintainer
   decisions; do not measure mixed-version rows.
...
4. If E4 is separately chosen later, follow `RELEASING.md`; before tagging,
   require the exact candidate commit's four remaining blocking release profiles to pass
...
7. For the separate GLIM comparison, prepare the exact public pair with
   `prepare_usability_scorecard_pair.py --verify-public`, then record it with
   `record_usability_scorecard_pair.py --require-ready`. Publish task-level
   evidence only; do not infer one overall winner.
8. Only after at least one comparable Docker PASS and one comparable source
