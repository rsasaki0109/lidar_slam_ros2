...
> `7b3cb9952ec73485e86f71bc71261ede0f77c842`
>
...
| Complete maintained product gate | RTK-SLAM acquisition synchronization: source-explicit graph 1,462 passed / 13 skipped / 11 existing ImageIO warnings; source-explicit lidarslam 1,040 passed; 2,502 total; strict MkDocs and changed-file Jazzy `ament_flake8` pass; latest public `662ac8f…` Actions pass 10 successful / 4 intentional skips / 0 failures / 0 pending; exact clean carrier `554e41b…` passes the canonical two-build bundle rehearsal with 263 manifest files, 11,938,413 archive bytes, and SHA-256 `1287d8af5627894aa37759ed031109d7204eca3608b5cf2ab499fdfed3a4cb06` | the new acquisition paths are fully test-, review-, and artifact-gated locally; the bundle is not a published release asset, and every later candidate must rerun rather than reuse its checksum |
| Actionable stable-release profile gate | exact gate implementation `45cfdcb…`; exact Leo implementation `92bb524…` records 0.139152 m Applanix cross-validation APE as `TARGET_MET`, keeps the source compressed bag unchanged, and automatically removes its private 2,506,907,648-byte playback view; the hard gate now reports four blocking `NO_DATA` profiles, preserves six report-only rows, and exits 2 | [exact evidence](leo-drive-compressed-playback-2026-08-16.md); this satisfies only the Leo cross-validation profile and grants no tag, Release, image, or E4 authority |
| RTK-SLAM acquisition recovery | exact implementation `2a997cb…`; all four official DB3/metadata identities and one detached eval commit are pinned, regular partial files are resumable, exact SHA-256 replaces the former 1% warning, and text/JSON plans are write- and network-free; the real smallest-profile request required 11,886,726,027 bytes, observed 6,409,396,224 bytes free, reported a 5,477,329,803-byte shortfall, exited 2 before side effects, and left its destination absent | [exact evidence](rtk-slam-acquisition-hardening-2026-08-17.md); this advances both RTK blockers from ambiguous acquisition to one safe external-disk retry, but downloads no large bag and satisfies no release profile |
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 314 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `78dc0990244e7b47e9f6b89d6ae71bebd9e5341d055c5a760d6416058b29119a` | the RTK-SLAM acquisition implementation, regression, and evidence paths belong to S5; every displayed verification group is self-contained for an ordinary terminal; exact coverage and the digest add no GitHub, release, posting, or community authority |
