...
| Safe operator interruption | exact implementation `6d1249e…`; a live `start` isolates and supervises the delegated runner, forwards one Ctrl-C as SIGINT, waits 20 seconds for cleanup/evidence and 10 seconds after termination before a forced reap, then projects the retained interrupt through the existing one-action recovery card; a real synthetic subprocess probe returns 130 and leaves no child process; 42 sensor-setup regressions, exact S3 lifecycle 77 and edit/merge 15, all 48 lower map-runner regressions, plus 42 docs/product, 25 support/installed, 21 option-contract, and 331 broad S6 regressions pass with changed-code `ament_flake8`, bytecode/JSON/patch hygiene, plan validation, and strict MkDocs | Ctrl-C no longer escapes as an ambiguous Python traceback or synthesizes verified success; the subprocess probe is not a real ROS mapping run, clean-host timing result, paired GLIM observation, or parity/superiority claim; no map, upload, network access, issue, or GitHub write is performed |
