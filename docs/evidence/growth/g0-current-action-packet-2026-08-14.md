...
| Safe operator interruption | original implementation `6d1249e…`, process correction `0301a0d…`, timeout-label correction `181b251…`, evidence-bound summary `8370ac5…`, quiet native-reader follow-up `d0e3361…`; product-dispatched native bag opens temporarily set only an unset `rosbag2_storage` logger to WARN and restore it after open, so direct and explicit expert levels plus WARN/ERROR remain visible; the exact-installed unchanged-default trial passed 7.02 GiB free / 5.00 GiB required, observed live RKO-LIO and graph nodes, then returned 130 in 1.545 seconds after one Ctrl-C with every descendant absent; six routine storage INFO lines fell to zero and stderr fell 1,200 → 180 bytes (85.00%), while one stop request, concise retained-evidence line, ACTION REQUIRED, Next, and Details remain; generic failure, failed command, generic session error, false timeout, log dump, traceback, and success cards stay at zero; four schemas and 18 checksums pass | expected stops are quiet without discarding stderr or weakening ordinary failure, warning, explicit expert-logging, or timeout diagnosis; the controlled mixed overlay means clean-host/package-manager, complete-map quality, paired GLIM, parity/superiority, upload, network, issue, and GitHub-write claims remain unavailable |
