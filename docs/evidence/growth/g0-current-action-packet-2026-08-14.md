...
| Issue-triage application packet | local follow-up converts the still-valid 29-row proposal into ordered, source- and evidence-hashed review actions: 23 closure drafts, 4 reproduction requests, 9 dependency reviews, and 1 monitor-only row (#422); the #69 draft now reflects the public Humble/Jazzy candidate, explains both leaf parameters and their resolution tradeoff, and retains the historical-bag and named-release limits; 21 focused regressions, the 1,061-test lidarslam gate, 35 graph docs/CLI regressions, strict MkDocs, and a complete authenticated GET-only live drift audit pass | stdout-only review aid; dated PR/CI/release claims still require maintainer recheck, every GitHub write authority remains false, no issue body, comment body, or author identity is retained, and no issue, label, comment, or state was changed |
...
| Publication slice plan | `PLAN_VALID_LOCAL_ONLY`; 327 paths / 7 slices from frozen baseline `3ed632e…`; the exact carrier's Git-derived follow-up count comes from `check_publication_slice_plan.py`; inventory SHA-256 `51cf5a5d500ec14fd48a4b11f812c6dc4aec98c55ca9a8e787dee849a25358c5` | the Odometry/TF connectivity and timing preflight belongs to S2/S6, while NTU and RTK acquisition remain in S5; the weekly growth snapshot, proposal content, and no-write issue-triage packet belong to S6; every displayed verification group is self-contained for an ordinary terminal; exact coverage and the digest add no mount, GitHub, release, posting, or community authority |
