...
is registered and built in testing for Humble and Jazzy, while the main
indexes still carry Humble `0.3.0` and Jazzy `0.2.0`; `ndt_omp_ros2` remains
absent. The maintained RKO-LIO fork has substantial product and research
patches, so an exact official binary must pass the pinned golden-path E2E on
both distributions before it can become the declared runtime dependency.
