...
> evidence contract are implemented, while its first scheduled artifact
> remains)**.
> This roadmap
...
bounded-filesystem workflow and evidence contract are implemented; its first
passing scheduled artifact and the first tagged publication of the new
rollback assets remain open, so Phase 3 remains in progress.
