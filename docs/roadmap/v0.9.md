...
> Humble/Jazzy GHCR release images with attestations landed; the public
> RKO-LIO binary boundary is resolved, while `ndt_omp_ros2` publication and
> cross-version upgrades remain open); Phase 3 in
...
RKO-LIO boundary status (2026-07-28): the public package is released for both
supported distributions and is now the declared runtime dependency of the
binary golden path. A
[clean Humble source-with-binary-RKO first-map trial](../evidence/upstream-rko-binary-first-map-2026-07-28.md)
passed a full public MID-360 bag with 8/8 Autoware verification checks. The
maintained fork and its fork-only profiles remain source/research features.
`ndt_omp_ros2`, Jazzy acceptance, package publication and upgrade testing
remain open, so Phase 2 is still in progress.
