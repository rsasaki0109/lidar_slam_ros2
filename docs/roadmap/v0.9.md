...
packages are present in Humble and Jazzy rosdistro, while `ndt_omp_ros2`
remains absent. The maintained RKO-LIO fork has substantial product and
research patches, so the official binary must pass the pinned golden-path E2E
on both distributions before it can become the declared runtime dependency.
