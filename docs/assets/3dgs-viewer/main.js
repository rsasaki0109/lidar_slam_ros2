...
// Local modification (lidarslam_ros2): frame the bundled koide map from one of
// its training views so the first frame is photoreal. Free-orbiting away from
// the capture path degrades (single-bag view-dependent overfit), so we open at
// a known-good pose and let the user nudge from there.
let defaultViewMatrix = [
    0.00121, 0.00249, 1.0, 0.0, -0.99995, -0.00945, 0.00123, 0.0, 0.00945,
    -0.99995, 0.00248, 0.0, -0.01452, -0.02072, 0.02123, 1.0,
];
let viewMatrix = defaultViewMatrix;
async function main() {
    // Local modification (lidarslam_ros2): start static at defaultViewMatrix
    // instead of auto-flying the upstream preset cameras (a different scene).
    let carousel = false;
    ...
    // Vendored from antimatter15/splat (MIT). Local modification: resolve the
    // ?url= splat relative to this page (the docs site) instead of the upstream
    // Hugging Face base, and default to the bundled lidarslam_ros2 map.
    const url = new URL(
        params.get("url") || "koide_lidar_primed.splat",
        location.href,
    );
