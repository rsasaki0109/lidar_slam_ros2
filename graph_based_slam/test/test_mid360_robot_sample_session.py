...
def _run_sample_session(tmp_path: Path, scenario: str) -> tuple[subprocess.CompletedProcess[str], dict, Path, Path]:
...
def test_sample_session_requires_force_for_existing_bag(tmp_path: Path):
    ...
    subprocess.run(command + ['--force'], check=True, capture_output=True, text=True, cwd=REPO_ROOT)
