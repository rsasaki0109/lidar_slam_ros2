...
if docker image inspect ghcr.io/autowarefoundation/autoware:universe-cuda >/dev/null 2>&1; then
  SIZE=$(docker images ghcr.io/autowarefoundation/autoware:universe-cuda --format '{{.Size}}')
  echo "  OK: universe-cuda (${SIZE})"
else
  echo "  FAIL: イメージが見つかりません"
  exit 1
fi
...
MAP_DIR="${AWSIM_DIR}/sample_map/nishishinjuku_autoware_map"
...
echo ""
echo "次のステップ:"
echo "  ターミナル1: bash run_awsim_autoware_demo.sh awsim"
echo "  ターミナル2: bash run_awsim_autoware_demo.sh autoware"
echo "  ターミナル3: bash run_awsim_autoware_demo.sh engage"
